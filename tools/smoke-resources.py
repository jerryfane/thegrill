...
def run(binary, out):
    ...
    try:
        # Finite readiness; this is the test-owned process, not a model/health endpoint.
        import selectors
        ...
        config = {
            "version": 1,
            "sources": [{"id": "fixture-process", "target": {"kind": "process", "pid": child.pid},
                         "ownership": {"kind": "process_address_space"}}],
        ...
        plan = {
            ...
            "gates": [{"source": "fixture-process", "metric": "resident_memory", "statistic": "sampled_maximum",
                       "max_regression_bps": 500, "max_reference_spread_bps": 500}],
        }
        ...
        for arm in plan["arms"]:
            for phase, identities in (("warmup", arm["warmup_ids"]), ("measured", arm["measured_ids"])):
                for index, identity in enumerate(identities):
                    ...
                    ticks = [s["readings"][0]["value"]["value"] for s in observation["snapshots"]]
                    assert ticks[-1] >= ticks[0], "native process counter reset"
        ...
        # Only this explicitly created finite fixture is terminated; never a service PID.
...
if __name__ == "__main__":
    ...
    if args.fixture:
        ...
        os.umask(0o077)
        run(args.binary, args.out.resolve())
    else:
