...
pub enum Mode { ReviewOnly }
impl Config {
    pub fn validate(&self, workload: &Workload) -> Result<()> {
        self.observer.validate()?;
        if self.version != 1 || workload.version != 6 || self.summaries.is_empty()
            || self.summaries.len() > 32 || self.retained_bytes == 0
            || self.retained_bytes > 1024 * 1024 * 1024 {
            return Err("serving resources require workload6 and bounded review-only configuration".into());
        }
        let mut keys = std::collections::BTreeSet::new();
        for gate in &self.summaries {
            if !self.observer.sources.iter().any(|s| s.id == gate.source)
                || gate.max_regression_bps != 0 || gate.max_reference_spread_bps != 0
        ...
        let groups = groups(&workload.waves(), workload.acquisition.as_ref().is_some_and(crate::acquisition::Protocol::conversation));
        ...
        if (groups.len() as u64).checked_mul(CAP as u64).is_none_or(|n| n > self.retained_bytes) {
            return Err("declared resource retention does not cover every acquisition artifact ceiling".into());
        }
        Ok(())
    }
}
...
pub fn groups(waves: &[WaveSpec], conversation: bool) -> Vec<Vec<u32>> {
    ...
    for (i, wave) in waves.iter().enumerate() {
        ...
        groups.last_mut().expect("group initialized").push(wave.index);
    }
    groups
}
#[derive(Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct Member { pub wave: u32, pub clock: StepClock }
#[derive(Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct Capture {
    pub version: u32,
...
impl<'a> Session<'a> {
    pub fn new(root: &'a Path, plan: &'a Plan, plan_hash: &'a str, origin: Instant) -> Self {
        Self { root, config: plan.workload.resources.as_ref(), plan_hash, origin,
            groups: groups(&plan.waves, crate::acquisition::conversation(&plan.workload)), active: None }
    }
    pub async fn begin(&mut self, wave: u32) -> Result<()> {
        let Some(config) = self.config else { return Ok(()); };
        if self.active.as_ref().is_some_and(|a| a.required.contains(&wave)) { return Ok(()); }
        self.finish(false).await?;
        let required = self.groups.iter().find(|g| g.first() == Some(&wave)).ok_or("resource acquisition boundary mismatch")?.clone();
        ...
        let task = tokio::spawn(async move {
            ...
            let mut timer = tokio::time::interval_at(tokio::time::Instant::now() + cadence, cadence);
            ...
            if sampling {
                if let Err(error) = observer.sample() { failure = Some(error); }
            }
            (observer, failure)
        });
        self.active = Some(Active { required, members: Vec::new(), stop, task });
        Ok(())
    }
    pub fn member(&mut self, wave: &Wave) {
        if let (Some(active), Some(clock)) = (&mut self.active, &wave.acquisition_clock) {
            active.members.push(Member { wave: wave.spec.index, clock: clock.clone() });
        }
    }
    pub async fn finish(&mut self, cancelled: bool) -> Result<()> {
        ...
        let (observer, failure) = active.task.await.map_err(|_| "resource observer task failed; no resource qualification")?;
        let complete = active.required.iter().copied().eq(active.members.iter().map(|m| m.wave));
        let interval = active.members.first().zip(active.members.last()).map(|(first,last)| resources::MeasuredInterval {
            clock: self.plan_hash.into(), started_us: first.clock.started_offset_us, settled_us: last.clock.settled_offset_us,
        });
        ...
        let observation = observer.finish(interval.unwrap_or(resources::MeasuredInterval {
            clock: self.plan_hash.into(), started_us: empty, settled_us: empty,
        }), cancelled || !complete || failure.is_some())?;
        let capture = Capture { version: 1, plan_sha256: self.plan_hash.into(), required_waves: active.required,
            members: active.members, complete_membership: complete, cancelled, failure, observation: Some(observation) };
        let bytes = serde_json::to_vec(&capture).map_err(|e| e.to_string())?;
        if bytes.len() > CAP { return Err("resource acquisition artifact exceeded admitted ceiling".into()); }
        evidence::write(&self.root.join(filename(capture.required_waves[0])), &bytes)?;
        evidence::sync(self.root)
    }
}
fn filename(first: u32) -> String { format!("resources-{first:06}.json") }
pub fn replay(root: &Path, plan: &Plan, plan_hash: &str, waves: &[Option<Wave>]) -> Result<Vec<Report>> {
    let Some(config) = &plan.workload.resources else { return Ok(Vec::new()); };
    let mut reports = Vec::new();
    for required in groups(&plan.waves, crate::acquisition::conversation(&plan.workload)) {
        let name = filename(required[0]);
        let path = root.join(&name);
        let exists = match std::fs::symlink_metadata(&path) { Ok(_) => true,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => false, Err(e) => return Err(e.to_string()) };
        let mut report = Report { required_waves: required.clone(), observed_waves: Vec::new(), complete_membership: false,
            cancelled: false, failure: Some("resource acquisition not retained".into()), measured: None, summaries: Vec::new(),
            raw_sidecar: name, sha256: None,
        ...
        if exists {
            ...
            if capture.version != 1 || capture.plan_sha256 != plan_hash || capture.required_waves != required
                || capture.members.len() > required.len()
                || !capture.members.iter().zip(&required).all(|(m,r)| m.wave == *r)
            ...
            for member in &capture.members {
                let wave = waves.get(member.wave as usize).and_then(Option::as_ref).ok_or("resource member has no published wave")?;
                if wave.acquisition_clock.as_ref() != Some(&member.clock) { return Err("resource step clock mismatch".into()); }
            }
            let observation = capture.observation.as_ref().ok_or("resource sidecar has no drained observation")?;
            resources::validate_observation(observation)?;
            if observation.config != config.observer || observation.clock.id != plan_hash
            ...
            if let Some((first,last)) = capture.members.first().zip(capture.members.last()) {
                if observation.measured != (resources::MeasuredInterval { clock: plan_hash.into(),
                    started_us: first.clock.started_offset_us, settled_us: last.clock.settled_offset_us }) {
                    return Err("resource interval is not complete acquisition span".into());
                }
                report.measured = Some(observation.measured.clone());
            } else if observation.measured.started_us != observation.measured.settled_us { return Err("undispatched acquisition has resource exposure".into()); }
            if (!capture.complete_membership || capture.cancelled || capture.failure.is_some())
                && !observation.failures.contains(&resources::Failure::Cancelled) {
            ...
            report.summaries = config.summaries.iter().map(|g| resources::summarize(observation, g)).collect();
