...
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
pub enum Source {
    #[serde(rename = "vllm-0.27.0-block-pool-v1")]
    VllmBlockPool,
    #[serde(rename = "vllm-487ecf187-block-pool-v1")]
    Vllm487BlockPool,
    #[serde(rename = "ordinary-lru-fixture-v1")]
    OrdinaryLruFixture,
}
impl Source {
        ...
        match self {
            Self::VllmBlockPool => &[
                (
                    ...
                    "51cad2fd425128a0ff433ca4685acfc022e40659153ea5d7180fc586e1eebb6c",
                ),
                (
                    "vllm/v1/core/kv_cache_manager.py",
                    "70f7f608c0963af155540630a5633483e5c19c0c0280dfd621760d5db2a419a6",
                ),
                (
                    "vllm/v1/request.py",
                    "6085b0668f41d56cd81ef483c06456f4ebe88bc1d46c8131d7134182d7893012",
                ),
            ],
            Self::Vllm487BlockPool => &[
                ...
                (
                    "vllm/v1/request.py",
                    "0287844f70eeaeb077d714e833a4b449a15e045a6516f8530182e357a5bec82f",
                ),
            ],
...
impl Plan {
    pub fn validate(&self, workload: &Workload) -> Result<()> {
        ...
        if self.metrics_labels.is_empty()
            || self.metrics_labels.len() > 16
            || self
                .metrics_labels
                .iter()
                .any(|(k, v)| k.is_empty() || k.len() + v.len() > 4096)
        {
            return Err("retention requires explicit bounded metrics2 label identity".into());
        }
        let order: Vec<&str> = std::iter::once(self.prime.as_str())
            .chain(self.pressure.iter().map(String::as_str))
            .chain(std::iter::once(self.probe.as_str()))
            .chain(self.recovery.iter().map(String::as_str))
            .collect();
        if order.len() != workload.cells.len()
            || !order
                .iter()
                .zip(&workload.cells)
                .all(|(id, c)| *id == c.case)
        ...
        let step = |id: &str| {
            workload
                ...
                .ok_or("retention phase has no actual sequence step")
        };
        let prime = step(&self.prime)?;
        if prime.parent.is_some() {
            return Err("retention prime must begin an actual history".into());
        }
        for id in &self.pressure {
            let pressure = step(id)?;
            if pressure.history == prime.history || pressure.parent.is_some() {
                return Err("pressure requires separate declared root histories".into());
            }
        }
        for id in std::iter::once(&self.probe).chain(&self.recovery) {
            let follow = step(id)?;
            if follow.history != prime.history || follow.parent.is_none() {
                return Err(
                    "probe/recovery must use an acquired parent in the prime history".into(),
                );
            }
        }
        Ok(())
...
fn accounting_complete(a: &crate::metrics::v2::Acquisition, plan: &Plan, expected: usize) -> bool {
    if a.snapshots != expected || a.complete != expected {
        return false;
    }
    let selected = |name: &str, labels: &BTreeMap<String, String>| {
        a.series
            .iter()
            .find(|s| s.identity.name == name && s.identity.labels == *labels)
    };
    for name in [
        ...
        if selected(name, &plan.metrics_labels)
            .is_none_or(|s| s.kind != "counter" || s.status != "continuous")
        {
            return false;
        }
    }
    for source in ["local_compute", "local_cache_hit", "external_kv_transfer"] {
        ...
        if selected("vllm:prompt_tokens_by_source_total", &labels)
            .is_none_or(|s| s.status != "continuous")
        {
            return false;
        }
    }
    let queries =
        selected("vllm:prefix_cache_queries_total", &plan.metrics_labels).and_then(|s| s.delta);
    let hits = selected("vllm:prefix_cache_hits_total", &plan.metrics_labels).and_then(|s| s.delta);
    if !matches!((queries,hits), (Some(q),Some(h)) if q >= h && h >= 0.0) {
        return false;
    }
    [
...
fn identity(file: &File) -> Result<String> {
    ...
    if !m.is_file() {
        return Err("journal is not an ordinary regular file".into());
    }
...
fn read_file(file: &File, cap: usize) -> Result<Vec<u8>> {
    ...
    loop {
        ...
        if bytes.len() + n > cap {
            return Err("journal byte ceiling exceeded".into());
        }
...
impl JournalWindow {
    pub fn begin(path: Option<&Path>, plan: &Plan) -> Result<Self> {
        ...
        let Some(path) = path else {
            window.failure = Some("no native eviction source selected".into());
            return Ok(window);
        };
        let opened = std::fs::OpenOptions::new()
            ...
            .open(path)
            .map_err(|e| e.to_string());
        match opened.and_then(|file| {
            let id = identity(&file)?;
            let bytes = read_file(&file, plan.journal_bytes as usize)?;
            Ok((file, id, bytes))
        }) {
            Ok((file, id, bytes)) => {
                window.file = Some(file);
                window.identity = Some(id);
                window.before = bytes;
            }
            Err(_) => {
                window.failure = Some("selected journal unavailable before acquisition".into())
            }
        }
        Ok(window)
    }
    pub fn finish(mut self, root: &Path, index: usize) -> Result<()> {
        ...
        if let Some(file) = &self.file {
            match read_file(file, self.plan.journal_bytes as usize) {
                Ok(bytes)
                    if bytes.starts_with(&self.before) && identity(file).ok() == self.identity =>
                {
                    after = bytes
                }
                _ => {
                    self.failure =
                        Some("journal changed, truncated or exceeded its byte ceiling".into())
                }
            }
        }
        let elapsed_us = u64::try_from(self.origin.elapsed().as_micros())
            .map_err(|_| "journal clock overflow")?;
        if elapsed_us > self.plan.max_window_us {
            self.failure = Some("journal acquisition window exceeded".into());
        }
        evidence::write(&root.join(format!("journal-{index:03}.jsonl")), &after)?;
        evidence::json(
            &root.join(format!("journal-{index:03}.json")),
            &Receipt {
                version: 1,
                source: self.plan.source,
                producer_sha256: self.plan.producer_sha256,
                provenance: "native-observed-unauthenticated-source".into(),
                before_bytes: self.before.len(),
                before_sha256: evidence::digest(&self.before),
                after_bytes: after.len(),
                after_sha256: evidence::digest(&after),
...
fn events(raw: &[u8], plan: &Plan) -> Result<Vec<Event>> {
    ...
    for line in raw[..raw.len() - 1].split(|b| *b == b'\n') {
        if line.len() > 65536 || events.len() >= plan.max_events as usize {
            return Err("journal event ceiling exceeded".into());
        }
        let event: Event = serde_json::from_slice(line).map_err(|e| e.to_string())?;
        if event.version != 1
            || event.source != plan.source
            || event.sequence as usize != events.len()
            || event.clock != "producer_monotonic_microseconds"
            || event.incarnation.len() != 32
        {
            return Err("journal source/order/clock mismatch".into());
        }
        if let Some(old) = events.last() {
            let old: &Event = old;
            if old.incarnation != event.incarnation
                || old.offset_us > event.offset_us
                || old.overhead_us > event.overhead_us
            {
                return Err("journal incarnation or monotonic clock changed".into());
            }
        }
        match &event.event {
            Kind::Store { cache_salt, keys }
            | Kind::Lookup {
                cache_salt, keys, ..
            } => {
            ...
            {
                return Err("invalid observed block allocation pressure".into());
            }
            Kind::Failure { reason } if reason.is_empty() || reason.len() > 256 => {
                return Err("invalid journal failure evidence".into());
            }
            _ => (),
        }
        events.push(event);
        match &events.last().expect("event retained").event {
            ...
            Kind::Evict { keys } => {
                for key in keys {
                    if !live.remove(key) {
                        return Err("eviction has no retained live insertion".into());
                    }
                }
            }
            Kind::Lookup {
                ...
                if (*cached_tokens == 0) != keys.is_empty()
                    || keys.iter().any(|k| !live.contains(k))
                {
            ...
            Kind::Installed { .. } if events.len() != 2 => {
                return Err("repeated hook installation".into());
            }
            _ => (),
        }
    }
    match events.first().map(|e| &e.event) {
        Some(Kind::Start {
            ...
            max_bytes,
            max_window_us,
        }) if producer_sha256 == &plan.producer_sha256
            && *max_events == plan.max_events
            && *max_bytes == plan.journal_bytes
            && *max_window_us == plan.max_window_us =>
        {
            ...
            if source_pins.len() != expected.len()
                || !expected
                    .iter()
                    .all(|(name, hash)| source_pins.get(*name).map(String::as_str) == Some(*hash))
            {
...
pub fn report(
    ...
    if receipt_path.try_exists().map_err(|e| e.to_string())? {
        let receipt: Receipt = serde_json::from_slice(&evidence::read(&receipt_path, 64 * 1024)?)
            .map_err(|e| e.to_string())?;
        let raw = evidence::read(
            &root.join(format!("journal-{index:03}.jsonl")),
            plan.journal_bytes as usize,
        )?;
        if receipt.version != 1
            || receipt.source != plan.source
            || receipt.producer_sha256 != plan.producer_sha256
            || receipt.provenance != "native-observed-unauthenticated-source"
            || receipt.after_bytes != raw.len()
            || receipt.after_sha256 != evidence::digest(&raw)
        {
            return Err("retention journal receipt mismatch".into());
        }
        if let Some(failure) = receipt.failure {
            unavailable.push(failure);
        } else if receipt.before_bytes > raw.len()
            || receipt.before_sha256 != evidence::digest(&raw[..receipt.before_bytes])
            || receipt.elapsed_us > plan.max_window_us
            || receipt.file_identity.is_none()
        {
            return Err("retention journal continuity mismatch".into());
        } else {
            if receipt.before_bytes == 0 || raw[receipt.before_bytes - 1] != b'\n' {
                unavailable
                    .push("journal acquisition start is not a complete event boundary".into());
            }
            boundary = raw[..receipt.before_bytes]
                .iter()
                .filter(|b| **b == b'\n')
                .count();
        ...
        unavailable.push("eviction journal absent".into());
    }
    let telemetry = run.metrics.as_ref().and_then(|m| m.acquisition.as_ref());
    let counters =
        telemetry.is_some_and(|a| accounting_complete(a, plan, run.plan.waves.len() * 2));
    if !counters {
        unavailable.push("complete metrics2 source continuity/accounting unavailable; preemption and miss are not eviction".into());
    }
    ...
    let lookup_indices: Vec<usize> = journal_events
        .iter()
        .enumerate()
        .skip(boundary)
        .filter_map(|(i, e)| matches!(e.event, Kind::Lookup { .. }).then_some(i))
        .collect();
    let mut allocations = Vec::new();
    let mut source_failed = false;
    for (i, event) in journal_events.iter().enumerate() {
        if let Kind::Failure { reason } = &event.event {
            source_failed = true;
    ...
    if lookup_indices.len() != published || boundary < 2 || published != run.plan.waves.len() {
        unavailable
            .push("journal request membership or complete acquisition coverage missing".into());
    }
    ...
    for group in crate::serving_resources::groups(&run.plan.waves, true) {
        ...
        for wave_index in &group {
            ...
            if !wave.is_some_and(|w| w.eligible) || !check.is_some_and(crate::sequence::passed) {
                group_qualified = false;
            }
            let expected_salt = check.map(|c| {
                format!(
                    "{}-{}",
                    crate::acquisition::namespace(
                        run.plan.cache_namespace.as_deref().unwrap_or(""),
                        spec.acquisition.expect("workload6")
                    ),
                    c.history
                )
            });
            ...
            if let Some(ei) = event_index {
                if let Kind::Lookup {
                    ...
                    cached_tokens,
                    keys,
                } = &journal_events[ei].event
                {
                    if expected_salt.as_deref() != Some(cache_salt) {
                        unavailable.push(
                            "journal lookup salt differs from actual acquired history".into(),
                        );
                    }
                    journal_hit = Some(*cached_tokens > 0);
                    if case == plan.prime {
                        prime_keys.extend(keys.iter().cloned());
                    }
                    // The previous phase ends immediately before this lookup;
                    // only allocation-driven removals before the probe count.
                    if case == plan.probe {
                        probe_hit = journal_hit;
                    }
                    if plan.recovery.last().map(String::as_str) == Some(case) {
                        recovery_hit = journal_hit;
                    }
                }
                let end = lookup_indices
                    .get(*wave_index as usize + 1)
                    .copied()
                    .unwrap_or(journal_events.len());
                for event in &journal_events[ei + 1..end] {
                    match &event.event {
                        Kind::Store { cache_salt, keys }
                            if case == plan.prime
                                && expected_salt.as_deref() == Some(cache_salt) =>
                        {
                            prime_keys.extend(keys.iter().cloned())
                        }
                        Kind::Evict { keys }
                            if case == plan.prime || plan.pressure.iter().any(|p| p == case) =>
                        {
                            evicted.extend(keys.iter().filter(|k| prime_keys.contains(*k)).cloned())
                        }
                        _ => (),
                    }
                }
    ...
    let satisfied = match plan.condition {
        Condition::ObserveOnly => false,
        Condition::EvictionAndRecovery => eviction_recovery,
        Condition::RetainedHit => retained_hit,
    };
    Ok(
        serde_json::json!({"version":1,"kind":"retention-observation-v1","source":plan.source,
...
mod tests {
    ...
    fn declaration() -> Plan {
        let mut capacity: crate::capacity::Plan =
            serde_json::from_str(include_str!("../examples/retention-capacity-v1.json")).unwrap();
    ...
    fn journal_requires_one_complete_selected_public_source_set() {
        ...
        for source in sources {
            plan.source = source;
            let pins: BTreeMap<String, String> = source
                .pins()
                .iter()
            ...
            for mask in 0..8 {
                let mixed: BTreeMap<String, String> = Source::VllmBlockPool
                    .pins()
                    .iter()
                    .enumerate()
                    .map(|(index, (name, old))| {
                        let hash = if mask & (1 << index) == 0 {
                            *old
                        } else {
                            Source::Vllm487BlockPool.pins()[index].1
                        };
                        ((*name).into(), hash.into())
                    })
                    .collect();
