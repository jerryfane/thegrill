...
pub enum Command {
    ...
    Import { source: PathBuf, #[arg(long)] out: PathBuf },
...
pub struct Slot { pub id: String, pub arm: Arm, pub warmup: bool, pub index: u32 }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct Clock {
    pub id: String,
...
impl Clock {
    fn validate(&self) -> Result<()> {
        if !short(&self.id) || self.kind != "monotonic" || self.units != "microseconds"
            || self.resolution_us == 0 || self.resolution_us > 1_000_000
    ...
    fn compatible(&self, other: &Self) -> bool {
        self.kind == other.kind && self.units == other.units
            && self.resolution_us == other.resolution_us && self.synchronization == other.synchronization
    }
}
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct ProcessIdentity { pub pid: u32, pub start_ticks: u64, pub boot_id: String }
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq, PartialOrd, Ord)]
#[serde(rename_all = "snake_case")]
pub enum Class { Engine, CacheServer, Filesystem, CompiledArtifactCache, Weights, PrefixOffload }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum ClassIdentity {
    Process { identity: ProcessIdentity },
    Artifact { content_sha256: String, generation: String },
}
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum Temperature { Cold, Warm, Unknown }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
...
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum Transition { Changed, Persisted }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct ClassPredicate { pub class: Class, pub transition: Transition }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
...
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct RequestItem { pub id: String }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum Study {
    Startup,
...
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum Target { Listening, Ready, FirstValidInference, Communication, Reload }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct Gate { pub target: Target, pub max_regression_bps: u32, pub max_reference_spread_bps: u32 }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct Plan {
    pub version: u32,
...
fn encode<T: Serialize>(value: &T) -> Result<Vec<u8>> { serde_json::to_vec(value).map_err(|e| e.to_string()) }
fn decode<T: serde::de::DeserializeOwned>(bytes: &[u8]) -> Result<T> { serde_json::from_slice(bytes).map_err(|e| e.to_string()) }
fn settings(plan: &Plan) -> model::RequestSettings {
    model::RequestSettings {
        profile: model::Profile::PortableChatV1, stream: false,
        output: model::OutputBudget { tokens: plan.request.max_tokens, mode: model::OutputMode::Cap },
        warmup_output: None, cache: model::Cache::Observe, temperature_milli: Some(0),
        top_p_milli: None, seed: None, thinking: None, thinking_control: None,
    }
}
fn request_body(plan: &Plan) -> Result<String> {
    #[derive(Serialize)]
    struct Body<'a> { model: &'a str, messages: [Message<'a>; 1], stream: bool, max_tokens: u32, temperature: u32,
        #[serde(skip_serializing_if = "Option::is_none")] cache_salt: Option<&'a str> }
    #[derive(Serialize)]
    struct Message<'a> { role: &'static str, content: &'a str }
    let salt = match &plan.study { Study::Startup => None, Study::RestartCache { identity, .. } => Some(identity.cache_salt.as_str()) };
    let body = serde_json::to_string(&Body { model: &plan.request.model,
        messages: [Message { role: "user", content: &plan.request.prompt }], stream: false,
        max_tokens: plan.request.max_tokens, temperature: 0, cache_salt: salt }).map_err(|e| e.to_string())?;
    if body.len() > model::REQUEST_CAP { return Err("startup request exceeds 2 MiB".into()); }
    Ok(body)
}
impl Plan {
    fn requests(&self, stage: Stage) -> Vec<String> {
        match (&self.study, stage) {
            (Study::RestartCache { post_restart, .. }, Stage::Reload) => post_restart.iter().map(|r| r.id.clone()).collect(),
            (_, Stage::Store) => vec!["store".into()],
            _ => vec!["first_inference".into()],
        }
    }
    pub fn validate(&self) -> Result<()> {
        if self.version != 1 || self.kind != "startup-study-v1" || !short(&self.study_id)
            || !hash(&self.collector_sha256) || !hash(&self.adapter_sha256) || !short(&self.setup_axis)
            || self.setup_sha256.iter().any(|h| !hash(h)) || self.setup_sha256[0] != self.setup_sha256[2]
            || self.slots.len() > 360 || self.slots.is_empty()
            || !(8..=1024).contains(&self.max_events) || !(1024..=CAP).contains(&self.event_bytes)
        ...
        if l.total_ms == 0 || l.total_ms > 600_000 || l.idle_ms == 0 || l.idle_ms > l.total_ms
            || l.response_bytes == 0 || l.response_bytes > 1024 * 1024
            || l.wave_buffer_bytes < l.response_bytes || l.wave_buffer_bytes > 16 * 1024 * 1024 {
            return Err("invalid startup HTTP bounds".into());
        }
        if self.request.model.is_empty() || self.request.model.len() > 256 || self.request.prompt.len() > model::REQUEST_CAP
            || self.request.expected_answer.is_empty() || self.request.expected_answer.len() > 65536 {
        ...
        if self.states.is_empty() || self.states.len() > 6
            || self.states.iter().any(|s| !state_classes.insert(s.class)) {
            return Err("startup cache-state exposure must be prospectively declared once per class".into());
        }
        ...
        for arm in [Arm::A, Arm::B, Arm::A2] {
            ...
            for warmup in [true, false] {
                ...
                while let Some(slot) = self.slots.get(offset).filter(|s| s.arm == arm && s.warmup == warmup) {
            ...
            if !(1..=20).contains(&local[0]) || !(3..=100).contains(&local[1]) || counts.is_some_and(|n| n != local) {
                return Err("each role needs equal prospective warmups and at least three measurements".into());
            }
            counts = Some(local);
        }
        if offset != self.slots.len() { return Err("startup slots must be ordered A/B/A2".into()); }
        if let Study::RestartCache { identity, post_restart, classes, .. } = &self.study {
            if identity.version != 1 || identity.prompt_sha256 != evidence::digest(self.request.prompt.as_bytes())
                || !hash(&identity.source_sha256) || !short(&identity.cache_salt)
                || identity.hash_seed_contract != "PYTHONHASHSEED=0:engine+cache_server"
                || post_restart.is_empty() || post_restart.len() > MAX_REQUESTS
                || post_restart[0].id != "first_reload" || classes.len() > 6 {
                return Err("invalid frozen restart cache identity or first-reload request list".into());
            }
            let mut names = BTreeSet::new();
            if post_restart.iter().any(|r| !short(&r.id) || !names.insert(&r.id)) {
                return Err("duplicate post-restart request".into());
            }
            let mut seen = BTreeSet::new();
            if classes.iter().any(|c| !seen.insert(c.class))
                || !classes.iter().any(|c| c.class == Class::Engine && c.transition == Transition::Changed)
                || !classes.iter().any(|c| c.class == Class::CacheServer && c.transition == Transition::Persisted) {
                return Err("restart requires changed engine and persisted cache-server predicates".into());
            }
            if self.metrics.is_none() { return Err("restart-cache requires explicit metrics2 collection".into()); }
        }
        let n = match &self.study { Study::Startup => 1, Study::RestartCache { post_restart, .. } => post_restart.len() };
        let scrape_allowance_us = if self.metrics.is_some() { 4_000_000 } else { 0 };
        if n * l.response_bytes > l.wave_buffer_bytes || n as u64 * (u64::from(l.total_ms) * 1000 + scrape_allowance_us) > self.deadline_us {
            return Err("request and scrape allowances exceed prospective memory/deadline".into());
        }
        if let Some(config) = &self.metrics { config.validate(n, self.local_http, self.auth_env.as_deref())?; }
        for (i, gate) in self.gates.iter().enumerate() {
            if gate.max_regression_bps > 9999 || gate.max_reference_spread_bps > 1_000_000
                || self.gates[..i].iter().any(|g| g.target == gate.target)
                || (matches!(gate.target, Target::Reload) && !matches!(self.study, Study::RestartCache { .. })) {
...
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum CacheResult { Stored, PersistedHit, Recomputed, Unavailable }
...
pub enum EventKind {
    ...
    CommunicationStart,
    CommunicationEnd,
    Controls { smoke_suppressed: bool, shape_warmup_suppressed: bool, hash_seed_contract: String },
    State { state: ClassState },
    RequestStart { id: String, body_sha256: String },
    InferenceComplete { id: String, response_sha256: String, cache_identity: Option<CacheIdentity>, cache_result: CacheResult },
    Failure { detail: String },
    /// The producer closes inference admission before writing this final event.
    Sealed { requests: usize },
}
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct Event {
...
pub struct DurationSample { pub target: Target, pub clock: Clock, pub duration_us: u64 }
struct Loaded { plan: Plan, capture: Capture, digest: String, inspection: Inspection }
fn process(pid: u32) -> Result<ProcessIdentity> {
    if pid == 0 { return Err("PID zero is not an attach target".into()); }
    let raw = evidence::read(Path::new(&format!("/proc/{pid}/stat")), 8192)?;
    let text = std::str::from_utf8(&raw).map_err(|_| "invalid process stat")?;
    let end = text.rfind(')').ok_or("invalid process stat comm boundary")?;
    let fields: Vec<_> = text[end + 1..].split_whitespace().collect();
    if fields.len() < 20 || matches!(fields[0], "Z" | "X" | "x") { return Err("observed process exited".into()); }
    let boot = evidence::read(Path::new("/proc/sys/kernel/random/boot_id"), 128)?;
    Ok(ProcessIdentity { pid, start_ticks: fields[19].parse().map_err(|_| "invalid process starttime")?,
        boot_id: std::str::from_utf8(&boot).map_err(|_| "invalid boot identity")?.trim().into() })
}
fn parse_events(raw: &[u8], plan: &Plan) -> Result<Vec<Event>> {
    if raw.len() > plan.event_bytes { return Err("event source byte budget exhausted".into()); }
    let mut events = Vec::new();
    let mut clocks: std::collections::BTreeMap<String, (Clock, u64)> = std::collections::BTreeMap::new();
    for line in raw.split_inclusive(|b| *b == b'\n') {
        if line.last() != Some(&b'\n') { return Err("incomplete journal event".into()); }
        if events.len() == plan.max_events { return Err("event count budget exhausted".into()); }
        ...
        if let Some((clock, offset)) = clocks.get(&e.clock.id) {
            if clock != &e.clock || *offset > e.offset_us { return Err("clock contract changed or time regressed".into()); }
        }
        clocks.insert(e.clock.id.clone(), (e.clock.clone(), e.offset_us));
        if let Some(previous) = events.last() {
            let previous: &Event = previous;
            if previous.clock.id == e.clock.id && (previous.clock != e.clock || previous.offset_us > e.offset_us) {
                return Err("clock changed or journal timestamps regressed".into());
            }
            if matches!(previous.event, EventKind::Sealed { .. }) { return Err("events after sealed admission".into()); }
        }
        events.push(e);
    }
    Ok(events)
...
struct Journal { path: PathBuf, inode: Option<(u64, u64)>, raw: Vec<u8> }
impl Journal {
    fn poll(&mut self, plan: &Plan) -> Result<()> {
        let file = match OpenOptions::new().read(true).custom_flags(libc::O_NOFOLLOW | libc::O_NONBLOCK).open(&self.path) {
            Ok(file) => file,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound && self.inode.is_none() => return Ok(()),
            Err(e) => return Err(format!("event source read: {e}")),
        };
        let m = file.metadata().map_err(|e| e.to_string())?;
        if !m.is_file() { return Err("event source is not a regular file".into()); }
        let identity = (m.dev(), m.ino());
        if self.inode.is_some_and(|old| old != identity) { return Err("event source incarnation changed".into()); }
        self.inode = Some(identity);
        let mut next = Vec::new();
        file.take(plan.event_bytes as u64 + 1).read_to_end(&mut next).map_err(|e| e.to_string())?;
        if !next.starts_with(&self.raw) { return Err("event source rewrote retained prefix".into()); }
        self.raw = next;
...
fn request_dir(root: &Path, index: usize) -> PathBuf { root.join(format!("request-{index:02}")) }

async fn collect_request(
    plan: &Plan, root: &Path, id: &str, index: usize, plan_hash: &str,
    window: std::ops::Range<Instant>, budget: &mut metrics::Budget,
    ...
    let ctx = metrics::CaptureCtx { origin, auth: metrics_auth.as_ref(), cancelled: None };
    ...
    let mut collected = wire::collect(client.clone(), request, plan.limits.clone(), settings(plan),
        wire::CollectContext { lane: 0, origin, deadline: Some(deadline), cancel,
            first_generated: None, tool_expectation: None }).await;
    let measured_duration = metrics::offset_us(origin, Instant::now())?.checked_sub(measured_origin)
    ...
    let reference = if let (Some(config), Some(before)) = (&plan.metrics, before) {
        ...
        let duration = before.duration_us.checked_add(after.duration_us).ok_or("metrics duration overflow")?;
        let publication = Instant::now();
        let mut reference = metrics::v2::publish(&dir, &metrics::v2::Receipt {
            version: 2, plan_sha256: plan_hash.into(), wave: index as u32,
            before, after, measured_origin_offset_us: measured_origin,
            measured_origin_unix_ms: measured_unix, measured_duration_us: measured_duration,
        })?;
        // Include snapshots and publication in the same charged overhead convention.
        reference.overhead_us = duration.checked_add(wire::us(publication)).ok_or("metrics overhead overflow")?;
        budget.finish_wave(reference.overhead_us, duration)?;
        Some(reference)
    } else { None };
    evidence::sync(&dir)?;
    Ok(RequestRecord { id: id.into(), body_sha256, attempt: collected.attempt, clock, metrics: reference })
}
...
pub fn capture(args: &CaptureArgs, stage: Stage, store: Option<&Path>) -> Result<Inspection> {
    ...
    if evidence::digest(&producer) != plan.adapter_sha256 { return Err("producer source pin mismatch".into()); }
    let binary = evidence::binary_digest()?;
    if binary != plan.collector_sha256 { return Err("collector binary pin mismatch".into()); }
    ...
    let previous_sha256 = if let Some(previous) = &args.previous {
        ...
        if loaded.capture.slot + 1 != args.slot || loaded.capture.stage == Stage::Store
            || loaded.capture.plan_sha256 != plan_hash {
            return Err("previous acquisition is not the preceding declared slot".into());
        }
        Some(loaded.digest)
    } else {
        if args.slot != 0 { return Err("later slot requires --previous retained acquisition".into()); }
        None
    };
    let stored = if let Some(path) = store {
        let loaded = load(path)?;
        if stage != Stage::Reload || loaded.capture.stage != Stage::Store
            || loaded.capture.plan_sha256 != plan_hash || loaded.capture.slot != args.slot
        ...
        if stage == Stage::Reload { return Err("reload requires retained store evidence".into()); }
        None
    };
    ...
    let store_sha256 = if let (Some(path), Some(loaded)) = (store, &stored) {
        ...
        Some(evidence::digest(&evidence::read(&args.out.join("store/capture.json"), CAP)?))
    } else { None };
    ...
    let mut journal = Journal { path: args.events.clone(), inode: None, raw: Vec::new() };
    ...
    runtime.block_on(async {
        if capture.failures.is_empty() {
            if let Err(e) = await_journal(&mut journal, &plan, &capture.engine, deadline, false).await {
        ...
        if capture.failures.is_empty() {
            for (index, id) in plan.requests(stage).iter().enumerate() {
                ...
                if Instant::now() + Duration::from_millis(u64::from(plan.limits.total_ms))
                    + Duration::from_secs(if plan.metrics.is_some() { 4 } else { 0 }) > deadline {
                    capture.failures.push("insufficient remaining request/scrape allowance".into());
                    break;
                }
                match collect_request(&plan, &args.out, id, index, &capture.plan_sha256, capture_origin..deadline, &mut budget).await {
                    Ok(record) => {
                        let failed = record.attempt.status != Status::Complete;
                        capture.requests.push(record);
                        if failed { capture.failures.push("declared inference failed; no replacement request".into()); break; }
                    }
                    Err(e) => { capture.failures.push(e); break; }
                }
            }
            if let Err(e) = await_journal(&mut journal, &plan, &capture.engine, deadline, true).await {
                capture.failures.push(e);
            }
        }
        if let Err(e) = journal.poll(&plan) { capture.failures.push(e); }
    });
    for id in [&capture.engine, &capture.cache_server].into_iter().flatten() {
    ...
    match (start, end) {
        (Some(a), Some(b)) if a.clock == b.clock && a.engine == b.engine => {
            if let Some(us) = b.offset_us.checked_sub(a.offset_us).filter(|n| *n > 0) {
                report.durations.push(DurationSample { target, clock: a.clock.clone(), duration_us: us });
            } else { add_reason(report, Outcome::Error, "nonpositive or reversed milestone duration"); }
        }
        _ => add_reason(report, Outcome::Inconclusive, format!("{target:?}: missing boundary or incompatible clock/incarnation")),
    }
}
fn response_valid(plan: &Plan, record: &RequestRecord, bytes: &[u8]) -> Result<()> {
    ...
    if a.response_sha256 != evidence::digest(bytes) || a.response_bytes != bytes.len()
        || a.lane != 0 || a.sequence.is_some() || record.body_sha256 != evidence::digest(request_body(plan)?.as_bytes()) {
        return Err("retained request/response integrity mismatch".into());
    }
    if a.status == Status::Complete {
        if !a.dispatched || a.http_status != Some(200) || a.finish_reason.as_deref() != Some("stop")
            || a.surplus_observed_bytes != 0 || !a.eligibility_errors.is_empty()
            || a.timing.settle_us == 0 || a.timing.settle_us > u64::from(plan.limits.total_ms) * 1000
            || a.usage.prompt_tokens.is_none_or(|n| n == 0)
            || a.usage.completion_tokens.is_none_or(|n| n == 0 || n > u64::from(plan.request.max_tokens)) {
        ...
        if response.pointer("/choices/0/message/content").and_then(serde_json::Value::as_str)
            != Some(plan.request.expected_answer.as_str()) {
            return Err("startup inference answer differs from the prospective exact answer".into());
        }
    }
    Ok(())
}
...
fn analyze(root: &Path, plan: &Plan, c: &Capture, raw: &[u8]) -> Result<Inspection> {
    ...
    let events = match parse_events(raw, plan) {
        Ok(events) => events,
        Err(e) => { add_reason(&mut report, Outcome::Error, format!("malformed journal: {e}")); Vec::new() }
    };
    let expected = plan.requests(c.stage);
    if c.requests.len() != expected.len() { add_reason(&mut report, Outcome::Inconclusive, "missing declared request acquisitions"); }
    let mut budget = metrics::Budget::default();
    ...
    for (index, record) in c.requests.iter().enumerate() {
        ...
        if expected.get(index) != Some(&record.id) { return Err("request order/identity mismatch".into()); }
        let dir = request_dir(root, index);
        ...
        if body != request_body(plan)?.as_bytes() { return Err("request bytes differ from frozen study identity".into()); }
        if let Err(e) = response_valid(plan, record, &response) { add_reason(&mut report, Outcome::Error, e); }
        if record.attempt.status != Status::Complete {
            add_reason(&mut report, Outcome::Inconclusive, format!("{}: {:?}", record.id, record.attempt.status));
        }
        match (&plan.metrics, &record.metrics) {
            (Some(config), Some(reference)) => {
                let summary = metrics::v2::load(&dir, reference, config, &c.plan_sha256, index as u32, &mut budget)?;
                let start = summary.receipt.measured_origin_offset_us;
                let finish = start.checked_add(summary.receipt.measured_duration_us).ok_or("metrics interval overflow")?;
                let a = &record.attempt.timing;
                if a.dispatch_offset_us < start || a.dispatch_offset_us.checked_add(a.settle_us).is_none_or(|end| end > finish) {
                ...
                if summary.receipt.before.status != metrics::v2::Status::Complete
                    ...
                    add_reason(&mut report, Outcome::Inconclusive, "required metrics2 snapshots unavailable");
                }
                if summary.accounting.iter().any(|a| a.status == "inconsistent") {
                    add_reason(&mut report, Outcome::Error, "provider accounting contradiction");
                }
                metric_waves.push(metrics::WaveReport::V2(summary));
            }
            (None, None) => (),
            _ => add_reason(&mut report, Outcome::Inconclusive, "required metrics companion missing"),
        }
    }
    let protocol = plan.metrics.clone().map(metrics::Protocol::V2);
    ...
    for event in &events {
        ...
        match &event.event {
            ...
            EventKind::CommunicationStart => {
                if communication_start.replace(event).is_some() { add_reason(&mut report, Outcome::Error, "duplicate communication start"); }
            }
            EventKind::CommunicationEnd => {
                if communication_end.replace(event).is_some() || communication_start.is_none() {
                    add_reason(&mut report, Outcome::Error, "communication substage boundary order");
                }
            }
            EventKind::Controls { smoke_suppressed, shape_warmup_suppressed, hash_seed_contract } => {
                if controls.replace((*smoke_suppressed, *shape_warmup_suppressed, hash_seed_contract)).is_some() || !starts.is_empty() {
                    add_reason(&mut report, Outcome::Error, "late or duplicate suppression controls");
                }
            }
            EventKind::State { state } => {
                if !classes.insert(state.class) { add_reason(&mut report, Outcome::Error, "duplicate class identity"); }
                if let Some(identity) = &state.identity {
                    let valid = match (state.class, identity) {
                        (Class::Engine, ClassIdentity::Process { identity }) => c.engine.as_ref() == Some(identity),
                        (Class::CacheServer, ClassIdentity::Process { identity }) => c.cache_server.as_ref() == Some(identity),
                        (Class::Engine | Class::CacheServer, _) => false,
                        (_, ClassIdentity::Artifact { content_sha256, generation }) => hash(content_sha256) && short(generation),
                        _ => false,
                    };
            ...
            EventKind::InferenceComplete { id, response_sha256, cache_identity, cache_result } => {
                ...
                if let Some(record) = c.requests.get(index) {
                    if record.id != *id || record.attempt.response_sha256 != *response_sha256
                        ...
                        add_reason(&mut report, Outcome::Error, "source inference is not supported by actual complete client response");
                    } else if index == 0 { first_inference = Some(event); }
                } else { add_reason(&mut report, Outcome::Error, "extra inference not in retained client requests"); }
                if let Study::RestartCache { identity, .. } = &plan.study {
                    if cache_identity.as_ref() != Some(identity) {
                        add_reason(&mut report, Outcome::Error, "changed or missing prompt/cache/hash-seed identity");
                    }
                    if let Some(record) = c.requests.get(index) {
                        match (cache_result, record.attempt.usage.cached_prompt_tokens) {
                            ...
                            (CacheResult::Unavailable, _) => add_reason(&mut report, Outcome::Inconclusive, "per-request cache accounting unavailable"),
                            _ => add_reason(&mut report, Outcome::Error, "source cache result contradicts actual response usage"),
                        }
                    }
                }
                if index == 0 { report.cache_result = Some(*cache_result); }
                finishes.push(id);
            }
            EventKind::Failure { detail } => add_reason(&mut report, Outcome::Inconclusive, format!("source failure: {detail}")),
    ...
    for (index, (id, body)) in starts.iter().enumerate() {
        if expected.get(index) != Some(*id) || **body != body_hash {
            add_reason(&mut report, Outcome::Error, "hidden, reordered or changed request bytes");
        }
    }
    for condition in &plan.states {
        match report.states.iter().find(|s| s.class == condition.class) {
            Some(state) if state.declared != condition.declared => {
                add_reason(&mut report, Outcome::Error, "cache-state declaration changed from prospective exposure");
            }
            Some(state) if state.observed == condition.required_observed => (),
            _ => add_reason(&mut report, Outcome::Inconclusive,
                format!("{:?} required observed cache state unavailable or refuted", condition.class)),
        }
    }
    for target in plan.gates.iter().map(|g| &g.target) {
        match target {
            ...
            Target::FirstValidInference => duration(&mut report, target.clone(), launched, first_inference),
            Target::Communication => duration(&mut report, target.clone(), communication_start, communication_end),
            Target::Reload if c.stage == Stage::Reload => {
                if let Some(record) = c.requests.first().filter(|r| r.attempt.status == Status::Complete) {
                    report.durations.push(DurationSample { target: Target::Reload, clock: record.clock.clone(),
                        duration_us: record.attempt.timing.settle_us });
                } else { add_reason(&mut report, Outcome::Inconclusive, "actual first-reload response unavailable"); }
            }
            Target::Reload => (),
        }
    }
    ...
    if plan.adapter == Adapter::UninstrumentedRecipeLogs {
        add_reason(&mut report, Outcome::Inconclusive, "recipe source mapping does not provide complete supported native milestone evidence");
    }
    Ok(report)
}
...
fn restart_predicates(report: &mut Inspection, store: &Inspection, plan: &Plan) {
    ...
    if store.outcome != Outcome::Pass { report.reasons.push("store acquisition is unqualified; retained under store/".into()); }
    let Study::RestartCache { classes, .. } = &plan.study else { return; };
    for predicate in classes {
        let before = store.states.iter().find(|s| s.class == predicate.class).and_then(|s| s.identity.as_ref());
        let after = report.states.iter().find(|s| s.class == predicate.class).and_then(|s| s.identity.as_ref());
        match (before, after) {
            (Some(a), Some(b)) if (a != b) == (predicate.transition == Transition::Changed) => (),
            (Some(_), Some(_)) => add_reason(report, Outcome::Error, format!("{:?} incarnation predicate refuted", predicate.class)),
            _ => add_reason(report, Outcome::Inconclusive, format!("{:?} before/after identity unavailable", predicate.class)),
        }
    }
    match report.cache_result {
        Some(CacheResult::PersistedHit) => (),
        Some(CacheResult::Recomputed) => add_reason(report, Outcome::Inconclusive, "actual first reload recomputed; persistence claim withheld"),
        _ => add_reason(report, Outcome::Inconclusive, "promised persisted-hit accounting unavailable"),
    }
    if let Some(record) = report.metrics.as_ref().and_then(|m| m.waves.first()) {
        if let metrics::WaveReport::V2(wave) = record {
            ...
            let supported = wave.cache.len() == 1 && wave.cache.first().is_some_and(|view| {
                view.external_queries.status == "available" && view.external_hits.status == "available"
                    && view.external_queries.delta.is_some_and(|n| n > 0.0)
                    && view.external_hits.delta.is_some_and(|n| n > 0.0)
            });
            if !supported {
                add_reason(report, Outcome::Inconclusive, "complete single-label external query/hit diagnostics unavailable or zero");
            }
        }
    } else { add_reason(report, Outcome::Inconclusive, "actual reload metrics2 evidence unavailable"); }
}
...
fn load(root: &Path) -> Result<Loaded> {
    ...
    if c.version != 1 || c.kind != "startup-observation-v1" || c.slot >= plan.slots.len()
        || c.plan_sha256 != evidence::digest(&plan_bytes) || c.collector_sha256 != plan.collector_sha256
        || c.requests.len() > MAX_REQUESTS || c.failures.len() > 64 || c.failures.iter().any(|s| s.len() > 4096)
        ...
        || !matches!((&plan.study, c.stage), (Study::Startup, Stage::Startup)
            | (Study::RestartCache { .. }, Stage::Store | Stage::Reload)) {
        return Err("startup capture identity, stage or bounds mismatch".into());
    }
    let producer = evidence::read(&root.join("producer.bin"), CAP)?;
    if evidence::digest(&producer) != plan.adapter_sha256 { return Err("source producer pin mismatch".into()); }
    let raw = evidence::read(&root.join("events.bin"), plan.event_bytes)?;
    if evidence::digest(&raw) != c.events_sha256 { return Err("event source digest mismatch".into()); }
    ...
    if let Some(hash) = &c.store_sha256 {
        ...
        if store_capture.stage != Stage::Store || store_capture.store_sha256.is_some()
            || evidence::digest(&store_bytes) != *hash {
            return Err("invalid nested store link".into());
        }
        let stored = load(&root.join("store"))?;
        if stored.capture.plan_sha256 != c.plan_sha256 || stored.capture.slot != c.slot
    ...
    let identity_bytes = if c.provenance == Provenance::Imported {
        ...
        if encode(&imported)? != encode(&c)? { return Err("import wrapper changed original acquisition facts".into()); }
        original
    } else { bytes };
    Ok(Loaded { plan, capture: c, digest: evidence::digest(&identity_bytes), inspection })
}

fn copy_capture(source: &Path, out: &Path, loaded: &Loaded, imported: bool) -> Result<()> {
    evidence::fresh(out)?;
    for (name, cap) in [("plan.json", CAP), ("producer.bin", CAP), ("events.bin", loaded.plan.event_bytes)] {
    ...
    for (index, record) in loaded.capture.requests.iter().enumerate() {
        ...
        evidence::fresh(&to)?;
        for (name, cap) in [("body.bin", model::REQUEST_CAP), ("response.bin", loaded.plan.limits.response_bytes)] {
        ...
        if record.metrics.is_some() {
            for name in ["metrics.json", "metrics-before.bin", "metrics-after.bin"] {
                evidence::write(&to.join(name), &evidence::read(&from.join(name), 1024 * 1024)?)?;
            }
        }
        evidence::sync(&to)?;
    }
    // Raw imported receipt is retained separately. The authoritative local wrapper is imported.
    let original = evidence::read(&source.join(if loaded.capture.provenance == Provenance::Imported {
        "imported-capture.json"
    } else { "capture.json" }), CAP)?;
    ...
    if loaded.capture.store_sha256.is_some() {
        ...
        receipt.store_sha256 = Some(evidence::digest(&evidence::read(&out.join("store/capture.json"), CAP)?));
    }
...
pub struct GateResult {
    pub gate: Gate, pub outcome: Outcome, pub reason: Option<String>,
    pub reference: Option<[envelope::Rational; 2]>, pub candidate: Option<[envelope::Rational; 2]>,
    pub counts: [usize; 3],
}
#[derive(Serialize)]
pub struct Comparison {
    pub version: u32, pub kind: &'static str, pub scope: &'static str,
    pub outcome: Outcome, pub reasons: Vec<String>, pub acquisitions: Vec<Inspection>, pub gates: Vec<GateResult>,
}
pub fn compare(plan_path: &Path, manifest: &Path) -> Result<Comparison> {
    ...
    if paths.len() > plan.slots.len() { return Err("extra undeclared startup acquisitions".into()); }
    let mut result = Comparison { version: 1, kind: "startup-comparison-v1",
        scope: "native-unauthenticated-domain-comparison-not-live-qualification", outcome: Outcome::Pass,
        reasons: Vec::new(), acquisitions: Vec::new(), gates: Vec::new() };
    ...
    for (index, path) in paths.iter().enumerate() {
        let loaded = load(path)?;
        if loaded.capture.plan_sha256 != evidence::digest(&source) || loaded.capture.slot != index
            || loaded.capture.stage == Stage::Store || loaded.capture.previous_sha256 != previous
        ...
        if let Some(engine) = &loaded.capture.engine {
            if !incarnations.insert((engine.boot_id.clone(), engine.pid, engine.start_ticks)) {
                return Err("repeated engine incarnation is not an independent startup acquisition".into());
            }
        }
        let usage: Vec<_> = loaded.inspection.request_usage.iter()
            .map(|u| (u.prompt_tokens, u.completion_tokens)).collect();
        if exposure.as_ref().is_some_and(|old| *old != usage) {
            result.outcome = result.outcome.max(Outcome::Inconclusive);
            result.reasons.push("actual prompt/output token exposure changed across acquisitions".into());
        }
    ...
    if paths.len() != plan.slots.len() {
        result.outcome = result.outcome.max(Outcome::Inconclusive);
        result.reasons.push("all declared acquisitions, including failed warmups, are required".into());
    }
    for gate in &plan.gates {
        ...
        for acquisition in &result.acquisitions {
            let duration = acquisition.durations.iter().find(|d| d.target == gate.target);
            if acquisition.outcome != Outcome::Pass || duration.is_none() { complete = false; }
            if let Some(duration) = duration {
                if clock.is_some_and(|c| !c.compatible(&duration.clock)) { complete = false; }
                clock = Some(&duration.clock);
                if !acquisition.slot.warmup {
                    let arm = match acquisition.slot.arm { Arm::A => 0, Arm::B => 1, Arm::A2 => 2 };
                    samples[arm].push(envelope::Rational { numerator: duration.duration_us, denominator: 1 });
                }
            }
        }
        ...
        let mut evaluated = GateResult { gate: gate.clone(), outcome: Outcome::Inconclusive,
            reason: None, reference, candidate, counts };
        if complete && counts.iter().all(|n| *n >= 3) {
            if let (Some(reference), Some(candidate)) = (reference, candidate) {
                match envelope::assess(reference, candidate, gate.max_regression_bps,
                    gate.max_reference_spread_bps, envelope::Direction::LowerBetter) {
                    Ok(assessment) => evaluated.outcome = match assessment.decision {
                    ...
                    Err(e) => {
                        evaluated.outcome = match e {
                            envelope::EnvelopeReason::InvalidRational | envelope::EnvelopeReason::InvalidBounds
        ...
        } else { evaluated.reason = Some("incomplete, unqualified or incompatible-clock acquisitions; no survivor-only gate".into()); }
...
pub fn execute(command: Command) -> Result<u8> {
    match command {
        ...
        Command::Reload { capture: args, store } => {
