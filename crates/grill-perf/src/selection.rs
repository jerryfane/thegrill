...
pub fn budgets(workload: &Workload, acquisitions: usize) -> Result<(usize, usize, usize)> {
    if workload.version == 6 {
        ...
        let scale = |n: u64| {
            usize::try_from(n)
                .ok()
                .and_then(|n| n.checked_mul(acquisitions))
                .ok_or_else(|| "acquisition budget overflow".to_string())
        };
        return Ok((
            scale(b.warmup_requests)?,
            scale(
                b.measured_requests
                    .checked_add(b.control_requests)
                    .ok_or("request budget overflow")?,
            )?,
            scale(b.output_token_ceiling)?,
        ));
    }
    if workload.version == 4 {
        ...
        for scenario in workload.schedule.iter().flatten() {
            for lane in &scenario.lanes {
                ...
                for (phase, trials) in [
                    (Phase::Warmup, scenario.warmup_trials),
                    (Phase::Measured, scenario.trials),
                ] {
                    let n = (trials as usize)
                        .checked_mul(acquisitions)
                        .ok_or("schedule request ceiling overflows")?;
                    ...
                    *count = count
                        ...
                        .ok_or("schedule request ceiling overflows")?;
                    tokens = n
                        .checked_mul(request.effective_output(phase).tokens as usize)
                        .and_then(|n| tokens.checked_add(n))
                        .ok_or("schedule output ceiling overflows")?;
                }
            }
        }
