...
pub(super) const ABI_SHA256: &str = "5a9ed049520b02354b74280f3a1e83b27292cf9f7958beb227461c0bc8676b99";
const LIBRARY: &CStr = c"libnvidia-ml.so.1";
// Maximum encoded trace is below this reservation: eight bounded calls, at
// most three fixed 96-byte buffers, fixed-size numeric records and identity pins.
...
struct MemoryInfo { total: c_ulonglong, free: c_ulonglong, used: c_ulonglong }
#[repr(C)]
union NvmlValue {
    d: c_double, ui: c_uint, ul: c_ulong, ull: c_ulonglong,
    sll: c_longlong, si: c_int, us: c_ushort,
}
#[repr(C)]
struct FieldValue {
    field_id: c_uint, scope_id: c_uint, timestamp: c_longlong,
    latency_us: c_longlong, value_type: c_int, code: c_int, value: NvmlValue,
}

#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum Api { Init, Version, Driver, Handle, UuidBefore, Memory, Power, UuidAfter, Shutdown }
impl Api {
    fn symbol(self) -> &'static CStr {
        match self {
            Self::Init => c"nvmlInitWithFlags", Self::Version => c"nvmlSystemGetNVMLVersion",
            Self::Driver => c"nvmlSystemGetDriverVersion", Self::Handle => c"nvmlDeviceGetHandleByUUID",
            Self::UuidBefore | Self::UuidAfter => c"nvmlDeviceGetUUID",
            Self::Memory => c"nvmlDeviceGetMemoryInfo", Self::Power => c"nvmlDeviceGetFieldValues",
...
enum Scalar { DoubleBits(u64), UnsignedInt(u32), UnsignedLong(u64), UnsignedLongLong(u64), SignedLongLong(i64), SignedInt(i32), UnsignedShort(u16) }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
struct PowerValue {
    field_id: u32, scope_id: u32, timestamp_us: i64, latency_us: i64,
    value_type: i32, code: i32, value: Option<Scalar>,
}
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(tag = "kind", content = "value", rename_all = "snake_case", deny_unknown_fields)]
enum Payload { Text(Vec<u8>), Handle(bool), Memory(MemoryInfo), Power(PowerValue) }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(tag = "state", rename_all = "snake_case", deny_unknown_fields)]
enum Returned { MissingSymbol, Called { code: i32, value: Option<Payload> } }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
struct Call { api: Api, result: Returned }
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LoadFailure { UnsupportedHost, LibraryUnavailable, ShutdownSymbolMissing }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
struct Trace {
    version: u32, abi_sha256: String, library: String,
    // This is the adapter ABI contract, not a fabricated observed driver version.
    abi: String, load_failure: Option<LoadFailure>, calls: Vec<Call>,
}
impl Trace {
    fn new() -> Self { Self { version: 1, abi_sha256: ABI_SHA256.into(),
        library: LIBRARY.to_string_lossy().into_owned(), abi: "linux-lp64-v1".into(),
        load_failure: None, calls: Vec::with_capacity(9) } }
    fn push(&mut self, api: Api, code: i32, value: Option<Payload>) -> bool {
        self.calls.push(Call { api, result: Returned::Called { code, value: if code == 0 { value } else { None } } });
        code == 0
    }
}
...
impl Library {
    fn open(name: &CStr) -> std::result::Result<Self, LoadFailure> {
        ...
        if handle.is_null() { return Err(LoadFailure::LibraryUnavailable); }
    ...
    fn collect_initialized(&self, uuid: &str, memory: bool, trace: &mut Trace) {
        for api in [Api::Version, Api::Driver] {
            let Some(symbol) = self.symbol(api, trace) else { return; };
            let get = unsafe { std::mem::transmute::<*mut c_void, Text>(symbol) };
            let mut bytes = vec![0u8; 80];
            let code = unsafe { get(bytes.as_mut_ptr().cast(), bytes.len() as c_uint) };
            if !trace.push(api, code, Some(Payload::Text(bytes))) { return; }
        }
        let Some(symbol) = self.symbol(Api::Handle, trace) else { return; };
        ...
        let Ok(uuid) = CString::new(uuid) else { return; };
        let mut device = std::ptr::null_mut();
        let code = unsafe { get(uuid.as_ptr(), &mut device) };
        if !trace.push(Api::Handle, code, Some(Payload::Handle(!device.is_null()))) || device.is_null() { return; }
        if !self.uuid(device, Api::UuidBefore, trace) { return; }
        ...
        let (code, payload) = if memory {
            ...
            let mut value = MemoryInfo { total: 0, free: 0, used: 0 };
            let code = unsafe { get(device, &mut value) };
            (code, Payload::Memory(value))
        } else {
            let get = unsafe { std::mem::transmute::<*mut c_void, Power>(symbol) };
            let mut field = FieldValue { field_id: POWER_INSTANT, scope_id: GPU_SCOPE,
                timestamp: 0, latency_us: 0, value_type: -1, code: -1, value: NvmlValue { ull: 0 } };
            let code = unsafe { get(device, 1, &mut field) };
            // Only read the active union member after BOTH return codes succeed.
            let value = if code == 0 && field.code == 0 { unsafe { match field.value_type {
                ...
                6 => Some(Scalar::UnsignedShort(field.value.us)), _ => None,
            } } } else { None };
            (code, Payload::Power(PowerValue { field_id: field.field_id, scope_id: field.scope_id,
                timestamp_us: field.timestamp, latency_us: field.latency_us,
                value_type: field.value_type, code: field.code, value }))
        };
        if !trace.push(api, code, Some(payload)) { return; }
        self.uuid(device, Api::UuidAfter, trace);
    }
    fn uuid(&self, device: Device, api: Api, trace: &mut Trace) -> bool {
        let Some(symbol) = self.symbol(api, trace) else { return false; };
...
impl Drop for Library {
    fn drop(&mut self) { unsafe { libc::dlclose(self.handle); } }
}
...
#[derive(Default)]
pub(super) struct NvmlSource { library: Option<std::result::Result<Library, LoadFailure>> }
impl NvmlSource {
    pub(super) fn read(&mut self, uuid: &str, memory: bool, cap: usize) -> Raw {
        let mut raw = Raw { name: "nvml.json".into(), bytes: Vec::new(), error: None };
        // Do not call a driver when the complete finite trace cannot be retained.
        if cap < TRACE_CAP { raw.error = Some(Failure::ByteBudget); return raw; }
...
pub(super) fn selected(target: &Target) -> Option<(&str, bool)> {
    match target {
        ...
        Target::NvidiaPower { uuid, .. } => Some((uuid, false)), _ => None,
    }
}
pub(super) fn valid_uuid(uuid: &str) -> bool {
    let Some(value) = uuid.strip_prefix("GPU-") else { return false; };
...
fn return_failure(code: i32) -> Failure {
    match code {
        ...
        10 => Failure::Deadline, 15 | 16 => Failure::SourceChanged,
        2 | 7 => Failure::Malformed, _ => Failure::Io,
    }
}
fn text(payload: &Payload, len: usize) -> std::result::Result<&str, Failure> {
    let Payload::Text(bytes) = payload else { return Err(Failure::Malformed); };
    if bytes.len() != len { return Err(Failure::Malformed); }
    let end = bytes.iter().position(|b| *b == 0).ok_or(Failure::Malformed)?;
    let value = std::str::from_utf8(&bytes[..end]).map_err(|_| Failure::Malformed)?;
    if value.is_empty() || !value.bytes().all(|b| b.is_ascii_graphic()) { return Err(Failure::Malformed); }
    Ok(value)
}
fn power(value: &PowerValue) -> std::result::Result<u64, Failure> {
    if value.field_id != POWER_INSTANT || value.scope_id != GPU_SCOPE { return Err(Failure::Malformed); }
    if value.code != 0 {
        return if value.value.is_some() { Err(Failure::Malformed) } else { Err(return_failure(value.code)) };
    }
    let milliwatts = match (&value.value, value.value_type) {
        ...
        (Some(Scalar::DoubleBits(_)), 0) | (Some(Scalar::SignedLongLong(_)), 4)
        | (Some(Scalar::SignedInt(_)), 5) | (Some(Scalar::UnsignedShort(_)), 6)
        | (None, 7..) | (None, i32::MIN..=-1) => return Err(Failure::UnsupportedMetric),
        _ => return Err(Failure::Malformed),
    };
    if value.timestamp_us <= 0 || value.latency_us < 0 { return Err(Failure::Malformed); }
    milliwatts.checked_mul(1000).ok_or(Failure::Overflow)
}

/// Pure replay: no dlopen, API calls, device handles, or clock mixing.
pub(super) fn parse(bytes: &[u8], uuid: &str, memory: bool)
    -> std::result::Result<(Option<String>, Vec<Reading>), Failure> {
    if bytes.len() > TRACE_CAP { return Err(Failure::ByteBudget); }
    let trace: Trace = serde_json::from_slice(bytes).map_err(|_| Failure::Malformed)?;
    if trace.version != 1 || trace.abi_sha256 != ABI_SHA256 || trace.library != "libnvidia-ml.so.1"
        || trace.abi != "linux-lp64-v1" || trace.calls.len() > 9 { return Err(Failure::Malformed); }
    if let Some(failure) = trace.load_failure {
        if !trace.calls.is_empty() { return Err(Failure::Malformed); }
        return Err(match failure { LoadFailure::LibraryUnavailable => Failure::Missing,
            LoadFailure::UnsupportedHost | LoadFailure::ShutdownSymbolMissing => Failure::UnsupportedMetric });
    }
    let order = [Api::Init, Api::Version, Api::Driver, Api::Handle, Api::UuidBefore,
        if memory { Api::Memory } else { Api::Power }, Api::UuidAfter];
    ...
    for call in &trace.calls {
        if call.api == Api::Shutdown {
            if !initialized || shutdown || (!stopped && position != order.len()) { return Err(Failure::Malformed); }
            shutdown = true;
            match call.result {
                Returned::Called { code, value: None } => if code != 0 { first_failure.get_or_insert(return_failure(code)); },
                _ => return Err(Failure::Malformed),
            }
            continue;
        }
        if shutdown || stopped || order.get(position) != Some(&call.api) { return Err(Failure::Malformed); }
        position += 1;
        let payload = match &call.result {
            Returned::MissingSymbol => { stopped = true; first_failure.get_or_insert(Failure::UnsupportedMetric); continue; }
            Returned::Called { code, value } if *code != 0 => {
                if value.is_some() { return Err(Failure::Malformed); }
                stopped = true; first_failure.get_or_insert(return_failure(*code)); continue;
            }
            Returned::Called { value, .. } => value,
        };
        if call.api == Api::Init {
            if payload.is_some() { return Err(Failure::Malformed); }
        ...
        let parsed = match call.api {
            ...
            Api::Handle => match payload { Payload::Handle(true) => Ok(()), Payload::Handle(false) => {
                stopped = true; Err(Failure::Malformed)
            }, _ => Err(Failure::Malformed) },
            Api::UuidBefore | Api::UuidAfter => text(payload, 96).and_then(|s| {
                if s == uuid && valid_uuid(s) { Ok(()) } else { Err(Failure::SourceChanged) }
            }),
            Api::Memory => match payload {
                Payload::Memory(m) if m.total > 0 && m.total != u64::MAX && m.free.checked_add(m.used) == Some(m.total) => {
                    readings = vec![reading(Metric::MemoryUsed, Unit::Bytes, Ok(m.used)),
                        reading(Metric::MemoryFree, Unit::Bytes, Ok(m.free)),
                        reading(Metric::MemoryLimit, Unit::Bytes, Ok(m.total))]; Ok(())
                }, _ => Err(Failure::Malformed),
            },
            Api::Power => match payload {
                Payload::Power(p) => power(p).map(|n| readings = vec![reading(Metric::Power, Unit::Microwatts, Ok(n))]),
                _ => Err(Failure::Malformed),
            },
            _ => Err(Failure::Malformed),
        };
        if let Err(failure) = parsed { first_failure.get_or_insert(failure); }
    }
    if initialized != shutdown || (!stopped && position != order.len()) || position == 0 { return Err(Failure::Malformed); }
    if let Some(failure) = first_failure { return Err(failure); }
    ...
    let identity = serde_json::to_vec(&(ABI_SHA256, uuid, version, driver)).map_err(|_| Failure::Malformed)?;
    Ok((Some(format!("nvml:{}", super::evidence::digest(&identity))), readings))
}
