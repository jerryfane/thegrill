...
pub enum ClockKind { StdInstantMonotonic }
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum ClockUnits { Microseconds }
impl Protocol {
    pub fn input_bytes(&self) -> usize {
        match self { Self::Flat { input_bytes } | Self::Conversation { input_bytes, .. } => *input_bytes }
    }
    pub fn history_bytes(&self) -> usize {
        match self { Self::Flat { .. } => 0, Self::Conversation { retained_history_bytes, .. } => *retained_history_bytes }
    }
    pub fn conversation(&self) -> bool { matches!(self, Self::Conversation { .. }) }
    pub fn measured(&self, case: &str) -> bool {
        match self { Self::Flat { .. } => true, Self::Conversation { measured_steps, .. } => measured_steps.iter().any(|s| s == case) }
    }
    pub fn counts(&self, cell: &Cell) -> (u32, u32) {
        match self {
            Self::Flat { .. } => (cell.trials, cell.warmup_trials),
            Self::Conversation { repetitions, warmup_repetitions, .. } => (*repetitions, *warmup_repetitions),
        }
    }
}
pub fn conversation(workload: &Workload) -> bool {
    matches!(workload.version, 2 | 5) || workload.acquisition.as_ref().is_some_and(Protocol::conversation)
}
pub fn input_cap(workload: &Workload) -> usize {
    workload.acquisition.as_ref().map_or(REQUEST_CAP, Protocol::input_bytes)
}
pub fn namespace(original: &str, identity: AcquisitionIdentity) -> String {
    let phase = match identity.phase { Phase::Warmup => "warmup", Phase::Measured => "measured" };
    crate::evidence::digest(format!("grill-acquisition-v1:{original}:{phase}:{}", identity.index).as_bytes())
}
pub fn validate(workload: &Workload) -> Result<()> {
    if workload.version != 6 {
        return if workload.acquisition.is_none() { Ok(()) } else { Err("acquisition requires workload6".into()) };
    }
    let protocol = workload.acquisition.as_ref().ok_or("workload6 requires explicit acquisition")?;
    ...
    match protocol {
        Protocol::Flat { .. } => {
            if !matches!(workload.request.profile, Profile::PortableChatV1 | Profile::VllmFixedV1) {
                return Err("flat acquisitions require a flat profile".into());
            }
        }
        Protocol::Conversation { repetitions, warmup_repetitions, measured_steps, retained_history_bytes, .. } => {
            if workload.request.profile != Profile::VllmConversationV3
                || !(1..=1000).contains(repetitions) || *warmup_repetitions > 20
                || !(1..=HISTORY_CAP).contains(retained_history_bytes)
                || measured_steps.is_empty() || measured_steps.len() > 128
...
pub fn waves(workload: &Workload) -> Vec<WaveSpec> {
    let protocol = workload.acquisition.as_ref().expect("validated acquisition");
    let mut result = Vec::new();
    let mut append = |cell: &Cell, phase, index| result.push(WaveSpec {
        index: result.len() as u32, phase, cell: cell.id.clone(), case: Some(cell.case.clone()),
        trial: index, concurrency: cell.concurrency, lanes: None,
        acquisition: Some(AcquisitionIdentity { phase, index }),
    });
    for phase in [Phase::Warmup, Phase::Measured] {
        match protocol {
            Protocol::Flat { .. } => for cell in &workload.cells {
                let (measured, warmup) = protocol.counts(cell);
                for index in 0..if phase == Phase::Warmup { warmup } else { measured } { append(cell, phase, index); }
            },
            Protocol::Conversation { repetitions, warmup_repetitions, .. } => {
                for index in 0..if phase == Phase::Warmup { *warmup_repetitions } else { *repetitions } {
                    for cell in &workload.cells { append(cell, phase, index); }
                }
            }
        }
    }
...
pub fn budget(workload: &Workload) -> Result<Budget> {
    ...
    let mut b = Budget { warmup_requests: 0, control_requests: 0, measured_requests: 0,
        output_token_ceiling: 0, encoded_input_byte_ceiling: 0, response_byte_ceiling: 0,
        tool_trace_byte_ceiling: 0, wall_time_ceiling_us: 0,
        retained_history_bytes: protocol.history_bytes(), serialized_bounds_not_rss_guarantees: true };
    for wave in workload.waves() {
        let n = u64::from(wave.concurrency);
        let count = if wave.phase == Phase::Warmup { &mut b.warmup_requests }
            else if protocol.measured(wave.case.as_deref().ok_or("missing case")?) { &mut b.measured_requests }
            else { &mut b.control_requests };
        *count = count.checked_add(n).ok_or("acquisition count overflow")?;
        b.output_token_ceiling = b.output_token_ceiling.checked_add(n * u64::from(workload.request.effective_output(wave.phase).tokens)).ok_or("output budget overflow")?;
        b.encoded_input_byte_ceiling = b.encoded_input_byte_ceiling.checked_add(n * protocol.input_bytes() as u64).ok_or("input budget overflow")?;
        b.response_byte_ceiling = b.response_byte_ceiling.checked_add(n * workload.limits.response_bytes as u64).ok_or("response budget overflow")?;
        if workload.cases.iter().find(|c| Some(c.id.as_str()) == wave.case.as_deref()).and_then(|c| c.step.as_ref()).is_some_and(|s| matches!(s.expect, crate::sequence::Expected::Tool { .. })) {
            b.tool_trace_byte_ceiling = b.tool_trace_byte_ceiling.checked_add(n * TOOL_TRACE_ALLOWANCE as u64).ok_or("trace budget overflow")?;
        }
        b.wall_time_ceiling_us = b.wall_time_ceiling_us.checked_add(u64::from(workload.limits.total_ms) * 1000).ok_or("time budget overflow")?;
    }
    Ok(b)
}
...
pub fn whole_samples(plan: &Plan, waves: &[Option<Wave>], phase: Phase) -> Result<Vec<u64>> {
    ...
    for index in 0..count { samples.push(whole_sample(plan, waves, AcquisitionIdentity { phase, index })?); }
    Ok(samples)
}
pub fn whole_sample(plan: &Plan, waves: &[Option<Wave>], identity: AcquisitionIdentity) -> Result<u64> {
        let mut start = None;
        let mut end = 0;
        let mut members = 0;
        ...
        if members != plan.workload.cells.len() { return Err("incomplete acquisition membership".into()); }
        end.checked_sub(start.ok_or("empty acquisition")?).filter(|n| *n > 0).ok_or_else(|| "nonpositive acquisition span".into())
}

#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq, Hash)]
#[serde(rename_all = "lowercase")]
pub enum Percentile { P95, P99 }
impl Percentile {
    pub fn floor(self) -> usize { match self { Self::P95 => 200, Self::P99 => 1000 } }
    pub fn nearest_rank(self, values: &mut [u64]) -> Option<u64> {
        if values.len() < self.floor() { return None; }
        values.sort_unstable();
        let p = match self { Self::P95 => 95usize, Self::P99 => 99 };
    ...
    for spec in workload.waves() {
        ...
        for lane in 0..spec.concurrency {
            ...
            if protocol.conversation() {
                let case = workload.cases.iter().find(|c| Some(c.id.as_str()) == spec.case.as_deref()).ok_or("unknown case")?;
                let step = case.step.as_ref().ok_or("missing step")?;
                if let Some(parent) = &step.parent {
                    bound = bound.checked_add(history_bounds.iter().find(|(id, _)| id == parent).ok_or("missing parent bound")?.1)
            ...
            if bound > protocol.input_bytes() { return Err("expanded acquisition input allowance exceeds input_bytes".into()); }
            let escaped = if protocol.conversation() {
                protocol.input_bytes().checked_mul(6).ok_or("reservation bound overflow")?
            } else { serde_json::to_string(&body).map_err(|e| e.to_string())?.len() };
            escaped_total = escaped_total.checked_add(escaped).ok_or("reservation bound overflow")?;
        }
        if escaped_total > 32 * 1024 * 1024 { return Err("acquisition exceeds reservation receipt bound".into()); }
    }
    Ok(())
}
...
pub fn settlement<'a>(mut attempts: impl Iterator<Item = &'a Attempt>) -> Result<u64> {
    attempts.try_fold(0u64, |end, a| a.timing.dispatch_offset_us.checked_add(a.timing.settle_us)
        .map(|n| end.max(n)).ok_or_else(|| "acquisition settlement overflow".into()))
}
pub fn eligibility_phase(workload: &Workload, phase: Phase) -> Phase {
    ...
    if workload.version == 6 && conversation(workload) { Phase::Measured } else { phase }
}
...
pub fn report(run: &crate::evidence::Loaded) -> Option<Report> {
    ...
    for spec in &run.plan.waves {
        ...
        let position = records.iter().position(|r| r.identity == identity && r.cell == cell);
        let index = position.unwrap_or_else(|| {
            records.push(Record { identity, cell, required_steps: Vec::new(), measured_steps: Vec::new(),
                missing_steps: Vec::new(), ineligible_steps: Vec::new(), started_offset_us: None,
                settled_offset_us: None, whole_conversation_wall_us: None, complete_eligible: run.history.count == 1 && !run.history.open && run.history.last_status.as_deref() == Some("completed") });
            records.len()-1
        });
        ...
        match run.waves.get(spec.index as usize).and_then(Option::as_ref) {
            Some(wave) => {
                ...
                } else { record.complete_eligible = false; }
            }
            None => { record.missing_steps.push(step.clone()); record.complete_eligible = false; }
        }
    }
