...
use super::super::{
    self as resources, Capture, Clock, ClockKind, ClockUnit, Gate, MeasuredInterval, Observation,
    Ownership, Phase, Provenance, ResourcesConfig, Role, Snapshot, Source, Statistic,
...
fn fixture(memory: bool) -> Trace {
    ...
    if memory {
        trace.push(
            Api::Memory,
            0,
            Some(Payload::Memory(MemoryInfo {
                total: 8192,
                free: 6144,
                used: 2048,
            })),
...
fn source(memory: bool) -> Source {
    Source {
        id: if memory { "memory" } else { "power" }.into(),
        target: if memory {
            Target::NvidiaMemory {
                uuid: UUID.into(),
                rank: Some(0),
        ...
        ownership: Ownership::SharedMemory {
            group: "synthetic-shared-device".into(),
        },
    }
}
fn snapshot(source: &Source, trace: &Trace, time: u64) -> Snapshot {
    let raw = Raw {
        name: "nvml.json".into(),
        bytes: serde_json::to_vec(trace).unwrap(),
        error: None,
    };
    let parsed = resources::parse_snapshot(source, std::slice::from_ref(&raw), None);
    let (incarnation, readings, failure) = match parsed {
        Ok((identity, readings)) => (identity, readings, None),
        Err(error) => (None, vec![], Some(error)),
    };
    Snapshot {
        source: source.id.clone(),
        clock: "fixture-clock".into(),
        started_us: time,
        observed_us: time,
        incarnation,
        raw: vec![raw],
...
fn observed(sources: Vec<Source>, traces: &[Trace]) -> Observation {
    let snapshots: Vec<_> = [1000, 2000]
        .into_iter()
        .flat_map(|t| {
            sources
                .iter()
                .zip(traces)
                .map(move |(s, trace)| snapshot(s, trace, t))
        })
        .collect();
    ...
    let config = ResourcesConfig {
        ...
        max_samples: 10,
        deadline_us: 10_000,
        per_sample_bytes: 32768,
        raw_total_bytes: 131072,
    };
    Observation {
        version: 1,
        kind: "resource-observation-v1".into(),
        provenance: Provenance::Imported,
        adapter: resources::native_adapter(&config).into(),
...
fn gate(memory: bool) -> Gate {
    Gate {
        source: if memory { "memory" } else { "power" }.into(),
        metric: if memory {
            Metric::MemoryUsed
        } else {
            Metric::Power
...
fn exact_memory_bytes_and_instant_power_milliwatt_conversion() {
    ...
    assert_eq!(
        memory,
        vec![
            reading(Metric::MemoryUsed, Unit::Bytes, Ok(2048)),
            reading(Metric::MemoryFree, Unit::Bytes, Ok(6144)),
            reading(Metric::MemoryLimit, Unit::Bytes, Ok(8192))
        ]
    );
    ...
    let Returned::Called {
        value: Some(Payload::Power(p)),
        ..
    } = &mut t.calls[5].result
    else {
        panic!()
    };
    p.value_type = 3;
    p.value = Some(Scalar::UnsignedLongLong(u64::MAX));
...
fn unsupported_memory_does_not_falsify_power_or_missing_ranks() {
    ...
    memory.calls[5].result = Returned::Called {
        code: 3,
        value: None,
    };
    ...
    validate_observation(&o).unwrap();
    assert_eq!(
        summarize(&o, &gate(true)).unavailable,
        Some(Failure::UnsupportedMetric)
    );
    assert_eq!(summarize(&o, &gate(false)).value, Some((125123, 1).into()));
    let mut missing = gate(false);
    missing.source = "rank-not-observed".into();
    assert_eq!(summarize(&o, &missing).unavailable, Some(Failure::Missing));
    let mut unknown = o;
    unknown.config.sources[1].ownership = Ownership::Unknown;
    assert_eq!(
        summarize(&unknown, &gate(false)).unavailable,
        Some(Failure::Ownership)
    );
}
...
fn native_admission_requires_full_uuid_and_device_scope() {
    ...
    for bad in [
        ...
        "GPU-00000000-0000-0000-0000-00000000000z",
    ] {
        config.sources[0].target = Target::NvidiaPower {
            uuid: bad.into(),
            rank: None,
        };
    ...
    assert!(config.validate().is_err());
    let parsed: Target =
        serde_json::from_str(&format!(r#"{{"kind":"nvidia_power","uuid":"{UUID}"}}"#)).unwrap();
    assert_eq!(
        parsed,
        Target::NvidiaPower {
            uuid: UUID.into(),
    ...
    assert!(
        serde_json::from_str::<Target>(&format!(
            r#"{{"kind":"nvidia_power","uuid":"{UUID}","index":0}}"#
        ))
        .is_err()
    );
}
...
fn raw_api_errors_have_no_zero_substitution_and_replay_requires_shutdown() {
    for (api, code, expected) in [
        (Api::Init, 9, Failure::Missing),
        (Api::Handle, 4, Failure::Permission),
        (Api::Power, 15, Failure::SourceChanged),
        (Api::Power, 999, Failure::Io),
        (Api::Shutdown, 10, Failure::Deadline),
    ] {
        ...
        if api != Api::Init && api != Api::Shutdown {
            trace.push(Api::Shutdown, 0, None);
        }
    ...
    let mut trace = fixture(false);
    trace.calls.truncate(6);
    trace.calls[5].result = Returned::MissingSymbol;
    trace.push(Api::Shutdown, 0, None);
...
fn field_status_type_scope_and_memory_consistency_are_independently_replayed() {
    ...
    assert_eq!(
        mutate(|p| {
            p.value_type = 0;
            p.value = Some(Scalar::DoubleBits(f64::NAN.to_bits()));
        }),
        Err(Failure::UnsupportedMetric)
    );
    assert_eq!(mutate(|p| p.timestamp_us = 0), Err(Failure::Malformed));
    let mut t = fixture(true);
    t.calls[5].result = Returned::Called {
        code: 0,
        value: Some(Payload::Memory(MemoryInfo {
            total: 8192,
            free: 8192,
            used: 1,
        })),
...
fn observed_uuid_and_runtime_changes_cannot_replay_as_same_source() {
    let mut t = fixture(false);
    t.calls[6].result = Returned::Called {
        code: 0,
        value: Some(buffer(OTHER_UUID, 96)),
    };
    assert_eq!(decode(&t, false), Err(Failure::SourceChanged));
    let base = fixture(false);
    let mut changed = fixture(false);
    changed.calls[2].result = Returned::Called {
        code: 0,
        value: Some(buffer("different-driver", 80)),
    };
    ...
    validate_observation(&o).unwrap();
    assert_eq!(
        summarize(&o, &gate(false)).unavailable,
        Some(Failure::SourceChanged)
    );
...
fn bounded_missing_library_and_budget_paths_never_load_nvidia() {
    ...
    let error = match Library::open(&path) {
        Err(error) => error,
        Ok(_) => panic!("nonexistent library opened"),
    };
    let mut source = NvmlSource {
        library: Some(Err(error)),
    };
...
fn cpu_shared_library_exercises_c_abi_without_driver_or_devices() {
    ...
    library.collect(UUID, false, &mut power);
    assert_eq!(
        decode(&power, false).unwrap().1,
        vec![reading(Metric::Power, Unit::Microwatts, Ok(125123000))]
    );
    let mut wrong_uuid = Trace::new();
    library.collect(OTHER_UUID, false, &mut wrong_uuid);
...
fn gpu_import_stays_imported_and_adapter_identity_is_source_specific() {
    ...
    let capture = Capture {
        version: 1,
        study_sha256: "2".repeat(64),
        acquisition_id: "fixture".into(),
        role: Role::A,
    ...
    assert_eq!(
        resources::native_adapter(&config),
