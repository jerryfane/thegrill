...
fn fragments() -> Vec<(u64, Vec<u8>)> {
    vec![
        ...
        (
            30,
            event(
                json!({"tool_calls":[{"index":0,"function":{"name":"up_fact","arguments":"{\"key\":"}}]}),
                None,
            ),
        ),
        (
            40,
            event(
                json!({"tool_calls":[{"index":0,"function":{"arguments":"\"harbor\"}"}}]}),
                None,
            ),
        ),
...
fn capture(chunks: Vec<(u64, Vec<u8>)>, interrupted: bool) -> (Attempt, Vec<u8>) {
    ...
    let mut timing = Timing {
        headers_us: Some(0),
        settle_us: 100,
        tool_stream_arrivals: Some(Vec::new()),
        ..Timing::default()
    };
    ...
    for (observed_us, chunk) in chunks {
        ...
        match parser.feed(&chunk, |bytes| {
            semantic.event(bytes, true, observed_us, &mut timing)
        }) {
            Ok(Some(consumed)) => {
                terminal_offset = Some(offset + consumed);
...
fn tool_stream_delayed_fragments_and_terminal_replay_exact_distinct_boundaries() {
    ...
    for field in [
        ...
        match field {
            ...
            _ => {
                changed.timing.tool_stream_arrivals.as_mut().unwrap().pop();
            }
        }
        assert!(
            sequence_tool(&changed, &body, expected()).is_err(),
            "{field}"
        );
    }
}
...
fn tool_stream_split_event_uses_chunk_completing_frame_not_initial_byte() {
    ...
    assert!(
        sequence_tool(&attempt, &body, expected())
            .unwrap()
            .is_some()
    );
}
...
fn tool_stream_incomplete_invalid_and_cancelled_candidates_never_qualify() {
    for kind in [
        ...
        "duplicate-id",
        "cancelled",
        "cancelled-after-done",
    ] {
        let mut chunks = fragments();
        match kind {
            "missing-done" | "cancelled" => {
                chunks.pop();
            }
            "incomplete-arguments" => {
                chunks.remove(3);
            }
            "trailing-value" => chunks.insert(
                4,
                (
                    45,
                    event(
            ...
            "duplicate-id" => chunks.insert(
                4,
                (
                    45,
                    event(json!({"tool_calls":[{"index":0,"id":"call_1"}]}), None),
                ),
            ),
            _ => (),
        }
...
fn tool_stream_terminal_boundary_excludes_coread_postterminal_fragments() {
    let mut chunks = fragments();
    chunks.last_mut().unwrap().1.extend(event(
        json!({"tool_calls":[{"index":0,"function":{"arguments":"junk"}}]}),
        None,
    ));
    let (attempt, body) = capture(chunks, false);
...
fn tool_stream_trace_contract_rejects_null_oversize_and_historical_fields() {
    ...
    oversized["tool_stream_arrivals"] = json!(vec![
        json!({"end_offset":1,"observed_us":1});
        TOOL_ARRIVAL_CAP + 1
    ]);
...
fn tool_stream_retained_history_overflow_withholds_validated_call() {
    ...
    assert!(
        semantic
            .event(raw.as_bytes(), true, 10, &mut timing)
            .is_err()
    );
